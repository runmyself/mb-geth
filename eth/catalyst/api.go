...
func NewConsensusAPI(eth *eth.Ethereum) *ConsensusAPI {
	...
	if api.eth.BlockChain().Config().Manba != nil { // don't start the api heartbeat, there is no transition
		return api
	}
	go api.heartbeat()
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	...
	} else if api.eth.BlockChain().Config().Manba == nil { // minor Engine API divergence: allow proposers to reorg their own chain
	...
	if payloadAttributes != nil {
		if api.eth.BlockChain().Config().Manba != nil && payloadAttributes.GasLimit == nil {
