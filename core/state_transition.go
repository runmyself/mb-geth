...
func (st *StateTransition) innerTransitionDb() (*ExecutionResult, error) {
	...
	// Check that we are post bedrock to enable mb-geth to be able to create pseudo pre-bedrock blocks (these are pre-bedrock, but don't follow l2 geth rules)
	// Note optimismConfig will not be nil if rules.IsOptimismBedrock is true
	if optimismConfig := st.evm.ChainConfig().Manba; optimismConfig != nil && rules.IsOptimismBedrock {
